...
class Scheduler {
      ...
      for (const j in fConfig.events) {
        ...
        schedule.scheduleJob(eventData.cron, () => {
          ...
          func(
            ...
            this._getContext(fConfig.id, fConfig.timeout),
            () => {}
          );
        });
      }
  ...
  _getContext(functionName, timeout = 10000) {
    ...
    return {
      ...
      getRemainingTimeInMillis: () => endTime - new Date().getTime() || 0
    };
  }
  ...
  _getFuncConfigs() {
    ...
    for (const funcName in inputfuncConfs) {
      ...
      const scheduleEvents = funcConf.events
        .filter((e) => e.hasOwnProperty("schedule") && e.schedule.enabled)
      ...
      if (scheduleEvents.length > 0) {
        funcConfs.push({
          ...
          timeout: funcConf.timeout ? funcConf.timeout * 1000 : 10000
        });
      }
    }
    return funcConfs;
